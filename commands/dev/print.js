...
module.exports = class printCommand extends commando.Command {
	constructor(client) {
		super(client, {
			name: 'print',
			group: 'util',
			memberName: 'print',
			description: 'Pretty prints executed JavaScript code result.',
			...
			args: [
				{
					key: 'script',
					prompt: 'Enter code to run and print now.',
					type: 'string'
				}
			]
		});
		...
		if(input) {
			return discord.splitMessage(tags.stripIndents`
				${editable ? `
					*Input*
					\`\`\`json
					${util.inspect(input, {compact: false, depth: 5, breakLength: 80})}
					\`\`\`` :
				''}
				*Executed in ${hrDiff[0] > 0 ? `${hrDiff[0]}s ` : ''}${hrDiff[1] / 1000000}ms.*
				\`\`\`json
				${util.inspect(inspected, {compact: false, depth: 5, breakLength: 80})}
				\`\`\`
			`, 1900, '\n', prepend, append);
		} else {
			return discord.splitMessage(tags.stripIndents`
